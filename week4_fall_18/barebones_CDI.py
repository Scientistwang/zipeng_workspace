...
def SSR(xa, xb, mu, M):
    ...
    nu = np.array([np.dot(xa, mu)/sum(xa),
                  np.dot(xb, mu)/sum(xb)])

    L = np.array([[np.dot(xa.T,np.dot(M,xa))/sum(xa), np.dot(xa.T,np.dot(M,xb))/sum(xa)],
                 [np.dot(xb.T,np.dot(M,xa))/sum(xb), np.dot(xb.T,np.dot(M,xb))/sum(xb)]])
    return nu, L
