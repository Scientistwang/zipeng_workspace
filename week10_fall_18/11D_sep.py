...
traj_2D_old =mmBC._11D_traj(0,([4,2]))
plt.plot(traj_2D_old[:,0], traj_2D_old[:,1],'b',label = 'Old Trajectory')
traj_2D_new =mmBC._11D_traj(Delta_M_4_2,([4,2]))
plt.plot(traj_2D_new[:,0], traj_2D_new[:,1],'r',label = 'New Trajectory')

num = 30
phase_dict = mmBC.get_ss_fates(num,0,([4,2]))
sep1 = mmBC.get_seperatrix_curve(phase_dict,num)
plt.plot(sep1[:,0],sep1[:,1],'--b')
steps = 4
color = ['r','r','r','r']
for i in range(steps):
        phase_dict = mmBC.get_ss_fates(num,Delta_M_4_2/steps*(i+1),([4,2]))
        sep1 = mmBC.get_seperatrix_curve(phase_dict,num)
